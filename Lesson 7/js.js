...
const snake = {
   ...
   * @param {int} rowsCount Количество строк поля, для корректного определения следующего хода
   * @param {int} colsCount Количество колонок поля, для корректного определения следующего хода
   */
  ...
  getNextStepHeadPoint() {
    ...
    switch (this.direction) {
      case 'up':
        return {x: firstPoint.x, y: (this.rowsCount + firstPoint.y - 1) % this.rowsCount};
      case 'right':
        return {x: (firstPoint.x + 1) % this.colsCount, y: firstPoint.y};
      case 'down':
        return {x: firstPoint.x, y: (firstPoint.y + 1) % this.rowsCount};
      case 'left':
        return {x: (this.colsCount + firstPoint.x - 1) % this.colsCount, y: firstPoint.y};
    }
  },
...
const game = {
   ...
   */
  renderScore() {
    document.getElementById('game-score').innerText = `Счет игры: ${this.score}`;
  },
};
